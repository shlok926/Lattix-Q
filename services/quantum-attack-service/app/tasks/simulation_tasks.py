...
def run_shors_simulation(self, key_size: int, include_noise: bool, shots: int):
    job_id = self.request.id
    try:
        r.setex(f"sim:job:{job_id}", 3600, json.dumps({"status": "RUNNING"}))
        algo = ShorsAlgorithm(key_size, include_noise, shots)
        result = algo.run()
        data = {
            "status": "COMPLETED",
...
def run_grovers_simulation(self, key_size: int, include_noise: bool):
    job_id = self.request.id
    try:
        r.setex(f"sim:job:{job_id}", 3600, json.dumps({"status": "RUNNING"}))
        algo = GroversAlgorithm(key_size, include_noise)
        result = algo.run()
        data = {
            "status": "COMPLETED",
            "result": asdict(result)
